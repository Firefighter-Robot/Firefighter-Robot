...
void LCD_WRITE_CHAR(unsigned char character){
	static char COUNT_CHAR = 0 ;	//used for counting the characters displayed
	//CONFIG_DATA_CTRL(OUTPUT,OUTPUT);	todo
	if(character){
...
//Check if the cursor at the end of the LCD row
	if (COUNT_CHAR == 15){	//if it at the edge of the first row
		GOTO_XY(0,1);		//Go to the second row
	}
	else if (COUNT_CHAR == 30){	//if it at the edge of the second row
		COUNT_CHAR = 0;
