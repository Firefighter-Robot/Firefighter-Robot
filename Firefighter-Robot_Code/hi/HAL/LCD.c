...
void LCD_init(void)
{
	...
	//init pin
<<<<<<< HEAD:Firefighter-Robot_Code/HAL/LCD.c
	MCAL_GPIO_Init(lcd_Data_Direction,&pincfg_LCD);
=======
	MCAL_GPIO_Init(GPIOB,&pincfg_LCD);
>>>>>>> 2202aee7558f4236c971f44f1a2401b16518f7b6:Firefighter-Robot_Code/hi/HAL/LCD.c
...
void LCD_Character( unsigned char character )
{
...
#endif
#ifdef _4_bit_mode
	MCAL_GPIO_WritePort(lcd_Data_Port, ((character & 0XF0)<<8));
	MCAL_GPIO_WritePin(lcd_control_Port,rs,GPIO_PIN_SET);           		  //  set rs to select data register
	MCAL_GPIO_WritePin(GPIOB,RW,GPIO_PIN_RESET);			  	    // R/W=0 i.e MODE Write
	MCAL_GPIO_WritePin(lcd_control_Port,en,GPIO_PIN_SET);			       // give high pulse to enable pin
	delay_ms(10);
	...
	MCAL_GPIO_WritePort(lcd_Data_Port, (character & 0X0F) << 12);             //sending lower nibble of 8 bit i.e 1byte
	MCAL_GPIO_WritePin(lcd_control_Port,rs,GPIO_PIN_SET);           		  //  set rs to select data register
	MCAL_GPIO_WritePin(GPIOB,RW,GPIO_PIN_RESET);			  	    // R/W=0 i.e MODE Write
	MCAL_GPIO_WritePin(lcd_control_Port,en,GPIO_PIN_SET);			   // give high pulse to enable pin
	delay_ms(10);
